...
class PMInterviewSimulator:
    def __init__(self):
        self.orchestrator = AoTOrchestrator(
            config=AoTConfig(skills=SKILLS, total_turn_limit=12, max_retries_per_skill=1)
        )
    ...
    async def run_simulation(self):
        print("=" * 60)
        print("  PRODUCT MANAGER — LIVE AoT INTERVIEW SIMULATION")
        print("=" * 60)
        
        candidate = CandidateState(
            candidate_id="pm_candidate_1",
            embedding=[0.0] * 384,
            skills={}
        )
